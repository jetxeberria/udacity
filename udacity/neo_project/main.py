...
def make_parser():
    ...
    query = subparsers.add_parser(
        'query',
        description="Query for close approaches that match a collection of\
                     filters.")
    filters = query.add_argument_group(
        'Filters',
        description="Filter close approaches by their attributes or the\
    ...
    filters.add_argument(
        ...
        help="Only return close approaches on or after the given date, in\
    ...
    filters.add_argument(
        ...
        help="In astronomical units. Only return close approaches that pass as\
              far or farther away from Earth as the given distance.")
    filters.add_argument(
        '--max-distance', dest='distance_max', type=float,
        help="In astronomical units. Only return close approaches that pass as\
              near or nearer to Earth as the given distance.")
    filters.add_argument(
        '--min-velocity', dest='velocity_min', type=float,
        help="In kilometers per second. Only return close approaches whose\
              relative velocity to Earth at approach is as fast or faster\
              than the given velocity.")
    filters.add_argument(
        '--max-velocity', dest='velocity_max', type=float,
        help="In kilometers per second. Only return close approaches whose\
              relative velocity to Earth at approach is as slow or slower than\
              the given velocity.")
    filters.add_argument(
    ...
    filters.add_argument(
        ...
        help="If specified, only return close approaches of NEOs that are\
              potentially hazardous.")
    filters.add_argument(
        '--not-hazardous', dest='hazardous', default=None,
        action='store_false',
        help="If specified, only return close approaches of NEOs that are not\
              potentially hazardous.")
    query.add_argument(
        '-l', '--limit', type=int,
        help="The maximum number of matches to return. Defaults to 10 if no\
              --outfile is given.")
    query.add_argument(
        '-o', '--outfile', type=Path,
        help="File in which to save structured results. If omitted, results\
              are printed to standard output.")

    repl = subparsers.add_parser(
        'interactive',
        description="Start an interactive command session to repeatedly run\
                     `interact` and `query` commands.")
    repl.add_argument(
...
class NEOShell(cmd.Cmd):
    ...
    def __init__(
            self, database, inspect_parser, query_parser, aggressive=False,
            **kwargs):
    ...
    def precmd(self, line):
        ...
        if changed:
            changed_files = ', '.join(str(f.relative_to(PROJECT_ROOT))
                                      for f in changed)
            print(
