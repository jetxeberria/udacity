...
from udacity.neo_project.extract import load_neos, load_approaches
from udacity.neo_project.database import NEODatabase
from udacity.neo_project.filters import create_filters, limit
from udacity.neo_project.write import write_to_csv, write_to_json
...
def main2():
    ...
    database = Path(repo_path, "data/neos.csv")
    cad_file = Path(repo_path, "data/cad.json")
    db = pd.read_csv(database)
    cad = filereader.read_json(cad_file)
    fields = cad["fields"]
    approaches = cad["data"]
    cl2015 = [
        a[7] 
        for a in approaches 
        if a[0] == "2002 PB" and a[3].startswith("2000-Jan-01")]
    return cad
