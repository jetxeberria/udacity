...
from tests.helpers import neos, neos_db, approach_170903, approaches
...
def test_filter_exact_date_g_approach_in_data_w_filtered_t_found(
        neos_db, approach_170903):
...
def test_filter_exact_short_date_g_approach_in_data_w_filtered_t_found(
        neos_db, approach_170903):
...
def test_filter_far_start_date_g_approach_in_data_w_filtered_t_not_found(
        neos_db):
...
def test_filter_early_end_date_g_approach_in_data_w_filtered_t_not_found(
        neos_db):
...
def test_filter_date_range_g_approach_in_data_w_filtered_t_found(neos_db):
    filters = create_filters(
        start_date="1900-Jan-01 03:13", end_date="1900-Jan-01 05:01")
    filtered_approaches = neos_db._approaches
    for f in filters:
        filtered_approaches = filter(f, filtered_approaches)
    approaches = list(filtered_approaches)
...
def test_filter_min_distance_g_approach_in_data_w_filtered_t_found(
        neos_db):
...
def test_filter_max_velocity_g_approach_in_data_w_filtered_t_found(
        neos_db):
...
def test_filter_min_velocity_g_approach_in_data_w_filtered_t_found(neos_db):
...
def test_filter_max_diameter_g_approach_in_data_w_filtered_t_found(neos_db):
...
def test_filter_min_diameter_g_approach_in_data_w_filtered_t_found(neos_db):
...
def test_filter_hazardous_g_approach_in_data_w_filtered_t_found(neos_db):
...
def test_filter_not_hazardous_g_approach_in_data_w_filtered_t_found(neos_db):
...
def test_limit_output_g_some_approaches_w_limited_t_reduced(neos_db):
    n = 3
